...
import { DBType, IDbCredentials } from './interface';
import { IErrorResponse } from 'src/common/interface';

@Injectable()
export class ConnectingService {
  private readonly logger = new Logger(ConnectingService.name);

  async getDbInstance(
    dbType: DBType,
    credentials: IDbCredentials,
  ): Promise<IErrorResponse | any> {
    try {
      if (!dbType)
        return {
          error: {
            type: 'Db connection failed',
            message: 'dbType is required',
            code: 400,
          },
        };
      ...
      switch (dbType.toLowerCase()) {
        ...
        case 'postgres':
        case 'postgresql':
          const client = new PgPool(credentials);
        ...
        default:
          return {
            error: {
              type: 'Db connection failed',
              message: 'Unsupported dbType',
              code: 400,
            },
          };
      }
    } catch (error) {
      this.logger.error('connection service catch error :' + error.message);
      throw new HttpException(error.message || 'connection failed', 400);
    }
  }
